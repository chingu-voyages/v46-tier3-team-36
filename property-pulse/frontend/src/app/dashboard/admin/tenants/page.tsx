'use client'
import { useGetUsersQuery, useUpdateUserMutation, useDeleteUserMutation } from "@/features/users/usersSlice";
...
const TenantsPage = () => {
	...
	const tenants = users.filter((user:User) => user.role === UserRoles.TENANT).toSorted((first:User, second:User) => {
	...
	const onUpdateFormSubmit = async (event:FormEvent) => {
		try {
						...
						{userIdEditing === tenant.id &&
							<form className="flex flex-col items-center p-3 gap-5" onSubmit={onUpdateFormSubmit}>
								...
								<div className="flex flex-row gap-5">
									<button value={tenant.id} type="button" onClick={onEditCloseClick} className="py-1 px-3 rounded-full text-black bg-green-400">Close</button>
									<button value={tenant.id} type="submit" className="py-1 px-3 rounded-full text-black bg-yellow-400">Save</button>
								</div>
							</form>
						}
					</li>
